...
const ShortKurti = () => {
  const [shortKurti, setShortKurti] = useState([]);
  ...
  useEffect(() => {
    fetch("/data/products.json")
      ...
      .then((data) => {
        if (data.short_kurti) {
          setShortKurti(data.short_kurti);
        }
      })
  ...
  const openProductPage = (product) => {
    navigate(`/short-kurti/product/${product.id}`);
  };
